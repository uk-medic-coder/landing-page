        ...
        if ($attname == 'style') {
            ...
            if (count($aMatch)) {
                foreach($aMatch[1] as $sMatch) {
                    $urlvalue = $sMatch;
                    tln_fixurl($attname, $urlvalue, $trans_image_path, $bternal_images);
...
function tln_fixurl($attname, &$attvalue, $trans_image_path, $bternal_images)
{
    $sQuote = '"';
...
function tln_fixstyle($body, $pos, $trans_image_path, $bternal_images)
{
    $me = 'tln_fixstyle';
    ...
    */
    ...
    if (count($aMatch)) {
        ...
        foreach($aMatch[1] as $sMatch) {
            // url value
            $urlvalue = $sMatch;
            tln_fixurl('style',$urlvalue, $trans_image_path, $bternal_images);
            $aValue[] = $sMatch;
            $aReplace[] = $urlvalue;
        }
    ...
    while (($curtag = tln_getnxtag($body, $curpos)) != false) {
        ...
        if ($tagname == "style" && $tagtype == 1){
            list($free_content, $curpos) =
                tln_fixstyle($body, $gt+1, $trans_image_path, $bternal_images);
...
function HTMLFilter($body, $trans_image_path, $bternal_images = false)
{
